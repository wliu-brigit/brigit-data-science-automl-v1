...
- **Live Snowflake e2e run** (`tests/e2e/test_snowflake_source_e2e.py`,
  written in step 3 but deferred here by wendao's call 2026-06-04): with
  VPN + `SNOWFLAKE_*` in `.env`, `AUTOML_E2E=1`, and a wendao-designated
  `dev_` table, exercise bootstrap → mint → attach → refresh-dedup live.
