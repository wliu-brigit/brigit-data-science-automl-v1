...
| 4 — flexible splits | [`step-4-flexible-splits.md`](step-4-flexible-splits.md) | not started | — | Heads-up from step-3 close (2026-06-04): (a) **range-API consumers exist that Task 5's file list predates** — `tests/e2e/test_snowflake_source_e2e.py` (config template, written in step 3), `projects/payment_routing/config.py`, and the scaffold's RUN_CONFIG *comment block* describing ranges (`scaffold.py` ~112–116) — sweep with `grep -rn "Splits("` rather than trusting the plan's list; (b) match file references by content, not line number (step 3 grew `pipeline.py` — adopt path + coercion — and rewrote `scaffold.py`); (c) suite baseline is now 554-green; (d) the live e2e + all VPN-dependent verification stays out of step 4's green gate — it's the batched tail-end session (wendao's call, see below). |
...
2. **Live notebook verification** on `example_homecredit` (notebooks 1–2
   ...
   cached outputs, and `notebooks/2_run_agent_automl.ipynb`'s
...
4. Revisit heads-up (e): whether `list_dataset_records`' swallow-to-`[]`
   can distinguish missing-folder from transport failure, now observable
   against the live proxy.
5. Move this effort `execution/ → archive/` per the docs lifecycle.
