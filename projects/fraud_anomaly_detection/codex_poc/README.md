...
```bash
uv run --group fraud python -m projects.fraud_anomaly_detection.codex_poc.export_neo4j_mirror --max-edges 10000
```
...
2. Deletes only this POC's prior Neo4j container/data under `codex_poc/out/`.
...
```bash
bash projects/fraud_anomaly_detection/codex_poc/scripts/stop_neo4j.sh
```

The setup uses `neo4j:5.26-community` by default. Override it if needed:

```bash
NEO4J_IMAGE=neo4j:latest bash projects/fraud_anomaly_detection/codex_poc/scripts/setup_neo4j.sh
```
...
```bash
cd projects/fraud_anomaly_detection/codex_poc/out/neo4j
