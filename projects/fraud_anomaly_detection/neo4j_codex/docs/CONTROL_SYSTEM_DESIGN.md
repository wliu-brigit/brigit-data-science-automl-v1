...
2026-06-13 brainstorming session (wendao + Claude). Built fresh in
`neo4j_codex/` as the forward Neo4j/control-loop comparison unit. Sign-off:
pending. Thresholds
...
**Definitions** (slow, reviewed, version-controlled, in-repo): canonical
scenarios in `register.yaml`, graph screens in
`control/discovery/graph_screen_catalog.py`, and plug-derivation rules. The
control-loop report is the current operator entry point; v3 can promote the
screen catalog into a richer method registry once the real method set is known.
The single tunable config (§6) lives here too. Editing one is a deliberate
reviewed act.
