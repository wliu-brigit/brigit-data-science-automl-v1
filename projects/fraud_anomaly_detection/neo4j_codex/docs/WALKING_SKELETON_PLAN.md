...
│   └── graph_screen_catalog.py  # reviewed graph screen metadata
...
└── control_loop_report.py # operator report: discovery → plugs → holdout

neo4j_codex/tests/control/
...
**Files:**
- Create: `neo4j_codex/control/__init__.py` (empty)
- Create: `neo4j_codex/control/contract.py`
- Create: `neo4j_codex/control/config.py`
- Test: `neo4j_codex/tests/control/test_contract.py`

- [ ] **Step 1: Write the failing test**

```python
# neo4j_codex/tests/control/test_contract.py
...
Run: `uv run --group fraud pytest projects/fraud_anomaly_detection/neo4j_codex/tests/control/test_contract.py -v`
Expected: FAIL (ModuleNotFoundError: control.contract).

- [ ] **Step 3: Write minimal implementation**

```python
# neo4j_codex/control/contract.py
...
```bash
git add projects/fraud_anomaly_detection/neo4j_codex/control/ projects/fraud_anomaly_detection/neo4j_codex/tests/control/test_contract.py
...
**Files:**
- Create: `neo4j_codex/control/finding_store.py`
- Create: `neo4j_codex/tests/control/conftest.py`
- Test: `neo4j_codex/tests/control/test_finding_store.py`

- [ ] **Step 1: Write the conftest fixture**

```python
# neo4j_codex/tests/control/conftest.py
...
```python
# neo4j_codex/tests/control/test_finding_store.py
import pandas as pd
from projects.fraud_anomaly_detection.neo4j_codex.control.contract import Finding, FindingSet
from projects.fraud_anomaly_detection.neo4j_codex.control.finding_store import FindingStore
...
Run: `uv run --group fraud pytest projects/fraud_anomaly_detection/neo4j_codex/tests/control/test_finding_store.py -v`
Expected: FAIL (cannot import FindingStore).

- [ ] **Step 4: Write minimal implementation**

```python
# neo4j_codex/control/finding_store.py
...
```bash
git add projects/fraud_anomaly_detection/neo4j_codex/control/finding_store.py projects/fraud_anomaly_detection/neo4j_codex/tests/control/
...
**Files:**
- Create: `neo4j_codex/control/discovery/__init__.py`
- Create: `neo4j_codex/control/discovery/scenario_method.py`
- Test: `neo4j_codex/tests/control/test_discovery.py` (scenario part)

- [ ] **Step 1: Write the failing test**

```python
# neo4j_codex/tests/control/test_discovery.py
from pathlib import Path

import pytest

from projects.fraud_anomaly_detection.neo4j_codex.control.discovery import DiscoveryMethod
...
Run: `uv run --group fraud pytest projects/fraud_anomaly_detection/neo4j_codex/tests/control/test_discovery.py -v`
Expected: FAIL (cannot import DiscoveryMethod).

- [ ] **Step 3: Write minimal implementation**

```python
# neo4j_codex/control/discovery/__init__.py
...
```python
# neo4j_codex/control/discovery/scenario_method.py
...
```bash
git add projects/fraud_anomaly_detection/neo4j_codex/control/discovery/ projects/fraud_anomaly_detection/neo4j_codex/tests/control/test_discovery.py
git commit -m "fraud control: discovery-method protocol + scenario adapter"
```

---

## Task 4: Graph discovery screens

**Forward status:** the old one-off graph adapter has been removed. Graph
evidence now enters through reviewed screen descriptors in
`control/discovery/graph_screen_catalog.py` and concrete query functions called
by `control/control_loop_report.py`. This better matches the operating model:
broad graph discovery can be visible in the report as `review_only` without
being eligible for plug derivation.

**Files:**
- `neo4j_codex/control/discovery/graph_screen_catalog.py`
- `neo4j_codex/control/control_loop_report.py`
- `neo4j_codex/tests/control/test_graph_screen_catalog.py`
- `neo4j_codex/tests/control/test_control_loop_report.py`

The sample proof is:

```bash
uv run --group fraud pytest -q \
  projects/fraud_anomaly_detection/neo4j_codex/tests/control/test_graph_screen_catalog.py \
  projects/fraud_anomaly_detection/neo4j_codex/tests/control/test_control_loop_report.py
```
...
**Files:**
- Create: `neo4j_codex/control/plug.py`
- Test: `neo4j_codex/tests/control/test_plug.py`
...
```python
# neo4j_codex/tests/control/test_plug.py
import duckdb
import pandas as pd
from projects.fraud_anomaly_detection.neo4j_codex.control.config import ControlConfig
...
Run: `uv run --group fraud pytest projects/fraud_anomaly_detection/neo4j_codex/tests/control/test_plug.py -v`
Expected: FAIL (cannot import plug.candidate_stats).

- [ ] **Step 3: Write minimal implementation**

```python
# neo4j_codex/control/plug.py
...
```bash
git add projects/fraud_anomaly_detection/neo4j_codex/control/plug.py projects/fraud_anomaly_detection/neo4j_codex/tests/control/test_plug.py
...
**Files:**
- Create: `neo4j_codex/control/holdout.py`
- Test: `neo4j_codex/tests/control/test_holdout.py`
...
```python
# neo4j_codex/tests/control/test_holdout.py
from projects.fraud_anomaly_detection.neo4j_codex.control.config import ControlConfig
...
Run: `uv run --group fraud pytest projects/fraud_anomaly_detection/neo4j_codex/tests/control/test_holdout.py -v`
Expected: FAIL (cannot import holdout.two_state_split).

- [ ] **Step 3: Write minimal implementation**

```python
# neo4j_codex/control/holdout.py
...
```bash
git add projects/fraud_anomaly_detection/neo4j_codex/control/holdout.py projects/fraud_anomaly_detection/neo4j_codex/tests/control/test_holdout.py
...
**Files:**
- Create: `neo4j_codex/control/monitoring.py`
- Test: `neo4j_codex/tests/control/test_monitoring.py`
...
```python
# neo4j_codex/tests/control/test_monitoring.py
import pandas as pd
from projects.fraud_anomaly_detection.neo4j_codex.control import monitoring
...
Run: `uv run --group fraud pytest projects/fraud_anomaly_detection/neo4j_codex/tests/control/test_monitoring.py -v`
Expected: FAIL (cannot import monitoring.holdout_effect).

- [ ] **Step 3: Write minimal implementation**

```python
# neo4j_codex/control/monitoring.py
...
```bash
git add projects/fraud_anomaly_detection/neo4j_codex/control/monitoring.py projects/fraud_anomaly_detection/neo4j_codex/tests/control/test_monitoring.py
git commit -m "fraud control: holdout monitoring stats (prevention / leakage / innocents)"
```

---

## Task 8: End-to-end control-loop report + smoke test

**Forward status:** the representative orchestrator described in the original
plan has been removed. The supported entry point is now
`neo4j_codex/control/control_loop_report.py`, which runs every scenario, graph
screening, graph promotion/status labeling, plug derivation, State A validation,
and holdout validation in one operator report.

**Files:**
- `neo4j_codex/control/control_loop_report.py`
- `neo4j_codex/tests/control/test_control_loop_report.py`

The smoke proof is the sample report test:

```bash
uv run --group fraud pytest \
  projects/fraud_anomaly_detection/neo4j_codex/tests/control/test_control_loop_report.py
```
...
`--include-status` controls displayed graph rows only. The underlying evaluation
still records all graph statuses and keeps review-only graph methods out of the
plug-derived final union until their metadata becomes promotion-safe.

- [ ] **Step 5: Commit**

```bash
git add projects/fraud_anomaly_detection/neo4j_codex/control/control_loop_report.py projects/fraud_anomaly_detection/neo4j_codex/tests/control/test_control_loop_report.py
...
**Files:**
- Modify: any `neo4j_codex/control/*.py` that imports from `neo4j_codex.archived`
- Test: `neo4j_codex/tests/control/test_no_archived_dep.py`

Policy (see `neo4j_codex/README.md`): borrowing from `archived/` *during*
...
If any module under `control/` imports from `...neo4j_codex.archived`, inline the
...
```python
# neo4j_codex/tests/control/test_no_archived_dep.py
import re
from pathlib import Path
...
def test_control_has_no_archived_dependency():
    ...
    for py in CONTROL.rglob("*.py"):
        if re.search(r"(from|import)\s+.*neo4j_codex\.archived", py.read_text()):
...
Run: `uv run --group fraud pytest projects/fraud_anomaly_detection/neo4j_codex/tests/control/test_no_archived_dep.py -v`
Expected: PASS (no `control/` module imports `archived/`).

- [ ] **Step 4: Run the full control suite, verify green**

Run: `uv run --group fraud pytest projects/fraud_anomaly_detection/neo4j_codex/tests/control/ -v`
Expected: PASS (all control tests).

- [ ] **Step 5: Commit**

```bash
git add projects/fraud_anomaly_detection/neo4j_codex/control/ projects/fraud_anomaly_detection/neo4j_codex/tests/control/test_no_archived_dep.py
