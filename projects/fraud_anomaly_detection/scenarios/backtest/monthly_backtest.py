...
Why this is cheap: the upstream feature snapshot is ~750 lines and ~1hr at
~10M rows because it computes the full feature set. The scenario predicates
need only TWO derived velocity features (users_on_bank_account_72h and
prior_advances_on_bank_account_7d), so this query computes just those.
...
/* 10. Scenario flags (one boolean per scenario + the union). */
flagged AS (
    SELECT
        advance_month,
        loan_amount,
